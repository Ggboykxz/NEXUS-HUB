...
// Revalide la page toutes les heures pour voir les nouveaux articles
export const revalidate = 3600;

async function getPublishedPosts(): Promise<BlogPost[]> {
  const { adminDb } = getAdminServices();
  const snap = await adminDb.collection('blogPosts')
    .where('status', '==', 'publié') // Sécurité: Ne lister que les articles publiés
  ...
  return snap.docs.map(doc => {
    ...
    return {
      ...data,
      id: doc.id,
      // Conversion du Timestamp Firestore en Date pour la sérialisation
                ...
                <Card className="flex flex-col h-full overflow-hidden transition-all hover:shadow-2xl border-none bg-muted/20 hover:bg-muted/30">
                  ...
                  <CardHeader className="p-6">
                    <div className="flex items-center gap-4 text-[10px] uppercase font-bold text-muted-foreground mb-3">
                      <span className="flex items-center gap-1.5"><Calendar className="h-3 w-3" /> {(post.publishedAt as Date).toLocaleDateString('fr-FR', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                      {/* Correction: Utilisation de `authorName` */}
