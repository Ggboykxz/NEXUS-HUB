...
export default function TranslationStudioPage(props: { params: Promise<{ storyId: string }> }) {
  ...
  // Simulation des bulles (En prod, on extrairait les textes des pages du chapitre)
  ...
  // 4. Approval Mutation (Only for Artist/Owner)
  const approveMutation = useMutation({
    mutationFn: async (transId: string) => {
      const ref = doc(db, 'stories', storyId, 'chapters', selectedChapterId, 'translations', transId);
      await updateDoc(ref, { status: 'approved', approvedAt: serverTimestamp() });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['chapter-translations', storyId, selectedChapterId] });
      toast({ title: "Traduction approuvée", description: "Cette version est désormais disponible pour les lecteurs." });
    }
  });
        ...
        <aside className="space-y-8">
          <Tabs defaultValue="dialogues" className="w-full">
            ...
            <TabsContent value="dialogues" className="space-y-4 pt-4 animate-in fade-in duration-500">
              <div className="flex items-center justify-between mb-4 px-2">
                <Label className="text-[10px] uppercase font-black text-primary tracking-widest">Langue de sortie</Label>
                <Select value={activeLang} onValueChange={setActiveLang}>
                  <SelectTrigger className="h-8 w-32 bg-white/5 border-none text-[10px] font-bold">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-stone-900 border-white/10">
            ...
            <TabsContent value="versions" className="space-y-4 pt-4 animate-in fade-in duration-500">
              <h3 className="text-xs font-black uppercase tracking-widest text-stone-500 px-2 mb-4">Historique des soumissions</h3>
              ...
              {loadingTrans ? (
                ...
                <div className="space-y-4">
                  {existingTranslations.map((trans: any) => (
                    <Card key={trans.id} className="bg-white/5 border-white/5 rounded-2xl p-5 space-y-4 group">
                      <div className="flex justify-between items-start">
                        ...
                        <Badge className={cn(
                          ...
                          trans.status === 'approved' ? "bg-emerald-500/10 text-emerald-500" : "bg-orange-500/10 text-orange-500"
                        )}>
                          {trans.status === 'approved' ? 'Approuvé' : 'En révision'}
                        </Badge>
                      </div>

                      {isOwner && trans.status === 'pending' && (
                        <div className="pt-2">
                          <Button 
                            onClick={() => approveMutation.mutate(trans.id)}
                            disabled={approveMutation.isPending}
                            className="w-full h-9 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[10px] font-black uppercase gap-2"
                          >
                            {approveMutation.isPending ? <Loader2 className="animate-spin h-3 w-3" /> : <><Check className="h-3.5 w-3.5" /> Approuver cette version</>}
                          </Button>
                        </div>
                      )}
                    </Card>
                ...
                <div className="text-center py-12 bg-white/[0.02] border border-dashed border-white/5 rounded-2xl">
                  <Clock className="h-8 w-8 text-stone-800 mx-auto mb-2" />
                  <p className="text-[10px] text-stone-600 uppercase font-black">Aucune version soumise</p>
                </div>
              )}
            </TabsContent>
          </Tabs>
          ...
          <Card className="bg-stone-900 border-none rounded-[2rem] p-8 space-y-4">
            ...
            <p className="text-[10px] text-stone-400 italic leading-relaxed">
              "L'approbation d'une traduction la rend immédiatement sélectionnable dans le sélecteur de langue du lecteur public. Vérifiez les nuances culturelles avant de valider."
            </p>
          </Card>
        </aside>
      </div>
